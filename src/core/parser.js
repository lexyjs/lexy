...
export default function parser (lang) {
  ...
  return function parse (tokens = [], opts = {}) {
    ...
    let table = [[new ParseState(lang.grammar[0].type, [], lang.grammar[0].pattern, 0, lang.grammar[0])]]
    ...
    tokens.forEach((token, index) => {
      ...
      for (let s = 0; s < table[index].length; s++) {
        let currentState = table[index][s]
        let states = []
  ...
  function computeClosures (state, i) {
    ...
    if (state.cd.length) {
      return lang.grammar.filter(r => r.type === state.cd[0]).map(r => new ParseState(r.type, [], r.pattern, i, r))
    }
  ...
  function computeShifts (state, token) {
    ...
    if (state.cd.length && state.cd[0] === token.type) {
      ...
      return [new ParseState(state.x, state.ab.concat([token.type]), state.cd.slice(1), state.j, state.rule)]
    }
  ...
  function computeReductions (state, i, table) {
    // When we have reached the rhs of a rule.
    if (!state.cd.length) {
      stack = [new Node(state.rule.execute ? state.rule.execute(stack) : stack, state.x)]
      ...
      let newStates = table[state.j]
        ...
        .map(s => new ParseState(s.x, s.ab.concat([state.x]), s.cd.slice(1), s.j, s.rule))
      return newStates
    }
